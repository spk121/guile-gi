...
guile-gi.tar: $(DISTFILES)
	tar --create --format=ustar --verbose --dereference --one-top-level --file=$@ $^
...
TEST_RESULTS = $(TESTS:.scm=.log)
%.log: %.scm
	LD_LIBRARY_PATH=. $(GUILE) -L src -L test $< > $@
...
BUILT_SOURCES = src/gi/__gi_gboxed.h \
	...
	src/gi/__gi_giargument.c
CLEANFILES = $($(check_DATA)) $(BUILT_SOURCES) doc/guile-gi.pdf doc/guile-gi.info
...
$(BUILT_SOURCES):  $(builddir)/fo_gen$(EXEEXT) src/gi/foreign-object-types.ini
...
.PHONY: check-syntax
check-syntax:
	$(CC) -std=c11 $(CFLAGS) -fsyntax-only $(CHK_SOURCES)
