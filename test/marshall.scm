...
(define-syntax-rule (boolarray-input f)
  (tap:test! (symbol->string (quote f))
             (f #(#t #f #t #t))
             #t))
...
(tap:test! "array-return"
           (list= eqv? '(-1 0 1 2) (int-vector->list (array-return))))

(tap:test! "array-return-etc"
           (receive (vals sum)
...
(tap:test! "array-gvariant-container-in"
           (let* ((v1 (variant:new-int32 27))
                  (v2 (variant:new-string "Hello"))
...
(tap:test! "array-gvariant-full-in"
           (let* ((v1 (variant:new-int32 27))
                  (v2 (variant:new-string "Hello"))
...
(parameterize ((tap:expect-fail #t))
  ...
  ;; the input vector.
  (tap:test! "garray-utf8-full-out-caller-allocated"
             (let ((x #("A" "B" "C")))
...
(tap:test! "array-zero-terminated-return-null"
           (vector-empty? (array-zero-terminated-return-null)))

(tap:test! "boolean-in-false"
           (boolean-in-false #f)
           #t)

(tap:test! "boolean-return-false"
           (not (boolean-return-false?)))

(tap:test! "boolean-out-false"
           (not (boolean-out-false)))

(tap:test! "boolean-in-true"
           (boolean-in-true #t)
           #t)

(tap:test! "boolean-return-true"
           (boolean-return-true?))

(tap:test! "boolean-out-true"
           (boolean-out-true))

(tap:test! "boolean-inout-false-true"
           (boolean-inout-false-true #f))

(tap:test! "boolean-inout-true-false"
           (not (boolean-inout-true-false #t)))

(tap:test! "init-function-null"
           (init-function))

(tap:test! "init-function-one"
           (receive (ret butlast)
               (init-function #("--help"))
             (and ret (not butlast))))

(tap:test! "init-function-two"
           (receive (ret butlast)
...
(tap:test! "utf8-as-uint8array-in"
           (utf8-as-uint8array-in (string->utf8 CONSTANT_UTF8))
           #t)

(tap:test! "utf8-dangling-out"
