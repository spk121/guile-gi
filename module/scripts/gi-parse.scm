...
(define (contents port lib allow-list deny-list header?)
  (let ((p (new-parser))
        (exports '())
    ...
    (catch #t
      (lambda ()
        ...
        (for-each (lambda (i)
		    (when (and (or (null? allow-list)
				   (member (info-name i) allow-list))
			       (or (null? deny-list)
				   (not (member (info-name i) deny-list))))
...
(define (makelib lib path allow-list deny-list overwrite no-header?)
    ...
    (let ((filename (string-append
      ...
      (if (and (not overwrite) (file-exists? filename))
          ...
          (let ((port (open-file filename "w")))
            (unless no-header?
              (header port lib))
            (contents port lib allow-list deny-list (not no-header?))
            (close port))))))

;; Must exclude those types that Guile-GI treats as internal data
;; types.
(define *deny-list*
  '(
    "Array"
       ...
       (else
        ...
        (let* ((outdir (or
               ...
               (libs (cons (list namespace (namespace-version namespace))
                           dependencies)))
          (for-each (lambda (lib)
                      (makelib lib outdir allow-list *deny-list* overwrite? no-header?))
                    libs)
      )))
      #t)
    (exit EXIT_SUCCESS)))
