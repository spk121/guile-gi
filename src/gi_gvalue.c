...
gi_gvalue_from_scm(GValue *value, SCM obj)
{
    ...
    case G_TYPE_POINTER:
    {
        if (SCM_POINTER_P(obj))
            ...
            g_value_set_object(value, gig_object_peek(obj));
        else
            return GI_GVALUE_WRONG_TYPE;
    }
    ...
    case G_TYPE_OBJECT:
        ...
        if (g_type_is_a(value_type, G_TYPE_OBJECT)) {
            ...
            else if (!G_TYPE_CHECK_INSTANCE_TYPE(gig_object_peek(obj), value_type))
                return GI_GVALUE_WRONG_TYPE;
            else {
                g_value_set_object(value, gig_object_peek(obj));
                return 0;
...
gi_gvalue_to_scm_structured_type(const GValue *value, GType fundamental, gboolean copy_boxed)
{
    ...
    case G_TYPE_BOXED:
    {
        if (G_VALUE_HOLDS(value, G_TYPE_VALUE)) {
            ...
            if (copy_boxed)
                return gig_object_ref(g_value_get_boxed(value));
            else
                return gig_object_take(g_value_get_boxed(value));
        }
    }
