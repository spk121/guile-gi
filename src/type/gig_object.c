...
#define FUNC "%make-gobject"
    ...
    if (scm_is_list(s_prop_keylist) && (scm_c_length(s_prop_keylist) != 0)) {
        ...
        for (size_t i = 0; i < n_prop; i++, iter = scm_cddr(iter)) {
            ...
            GParamSpec *pspec = G.object_class_find_property(_class, keys[i]);
            if (!pspec) {
    ...
    obj = G.object_new_with_properties(type, n_prop, keys, values);
    scm_dynwind_end();
...
get_paramspec(const GObject *self, const char *prop)
{
    ...
    return G.object_class_find_property(oclass, prop);
}
...
static void
make_new_signal(GigSignalSpec *signal_spec, void *user_data)
{
    GType instance_type = GPOINTER_TO_SIZE(user_data);
    G.signal_newv(signal_spec->signal_name, instance_type, signal_spec->signal_flags, NULL,     /* closure */
...
gig_user_object_get_property(GObject *object, unsigned property_id, GValue *value,
                             GParamSpec *pspec)
{
    GValue *properties = G.object_get_qdata(object, gig_user_object_properties);
    GValue *property = properties + property_id - 1;
    G.value_copy(property, value);
}
                             ...
                             const GValue *value, GParamSpec *pspec)
{
    GValue *properties = G.object_get_qdata(object, gig_user_object_properties);
    GValue *property = properties + property_id - 1;
    G.param_value_convert(pspec, value, property, FALSE);
}
...
gig_user_object_dispose(GObject *object)
{
    ...
    G.type_class_unref(_parent_class);
}
...
gig_user_class_init(GObjectClass *_class, void *class_info)
{
    ...
    for (size_t i = 0; i < init_info->n_properties; i++)
        G.object_class_install_property(_class, i + 1, init_info->properties[i]);
}
...
gig_user_object_init(GTypeInstance *instance, void *class_ptr)
{
    ...
    GType parent_type = G.type_parent(type);
    unsigned n_properties;
    GParamSpec **properties;
    GTypeQuery query;

    G.type_query(parent_type, &query);
    ...
    for (unsigned i = 0; i < n_properties; i++) {
        ...
        _default = G.param_spec_get_default_value(properties[i]);
        G.value_init(instance_properties + i, properties[i]->value_type);
        G.value_copy(_default, instance_properties + i);
    }
}
                       ...
                       size_t n_signals, GigSignalSpec **signals)
{
    ...
    new_type = G.type_register_static(parent_type, type_name, &type_info, 0);
...
gig_i_scm_define_type(SCM s_type_name, SCM s_parent_type, SCM s_properties, SCM s_signals)
{
    ...
    GType parent_type;
    GType new_type;
    size_t n_properties = 0, n_signals = 0;
              ...
              GQuark *c_detail)
{
    ...
    *c_signal = G.signal_lookup(name, G_OBJECT_TYPE(self));
    free(name);
    ...
    G.signal_query(*c_signal, query_info);
    ...
    if ((query_info->signal_flags & G_SIGNAL_DETAILED) && scm_is_symbol(detail)) {
        ...
        *c_detail = G.quark_from_string(_detail);
        free(_detail);
    }
    else
        *c_detail = 0;
...
gig_i_scm_connect(SCM self, SCM signal, SCM sdetail, SCM callback, SCM s_after, SCM reserved)
{
    ...
    handlerid = G.signal_connect_closure_by_id(obj, sigid, detail, closure, after);
