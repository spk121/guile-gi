...
gig_irepository_get_immediate_dependencies(SCM s_namespace)
{
    ...
    c_dependencies = g_irepository_get_immediate_dependencies(nullptr, c_namespace);
    free(c_namespace);
    return c_strings_to_scheme_and_free(c_dependencies);
}
...
gig_irepository_is_registered(SCM s_namespace, SCM s_version)
{
    ...
    gboolean c_ret = g_irepository_is_registered(nullptr, c_namespace, c_version);
    free (c_version);
    free(c_namespace);
...
gig_irepository_require(SCM s_namespace, SCM s_version, SCM s_flags)
{
	...
	tl = g_irepository_require(nullptr, c_namespace, nullptr, c_flags, &error);
    else {
	c_version = scm_to_utf8_string(s_version);
	tl = g_irepository_require(nullptr, c_namespace, c_version, c_flags, &error);
	free(c_namespace);
    }
    free(c_version);
...
SCM
gig_irepository_require_private(SCM s_typelib_dir, SCM s_namespace, SCM s_version, SCM s_flags)
{
    ...
    tl = g_irepository_require_private(nullptr, c_typelib_dir, c_namespace, c_version, c_flags, &error);
    free(c_typelib_dir);
    free(c_namespace);
    free(c_version);
...
gig_irepository_find_by_name(SCM s_namespace, SCM s_name)
{
    ...
    GIBaseInfo *c_info = g_irepository_find_by_name(nullptr, c_namespace, c_name);
    free(c_namespace);
    free(c_name);
    if (c_info != nullptr)
	return _scm_from_gibaseinfo(c_info);
...
gig_base_info_get_attributes (SCM s_info)
{
    ...
    {
	ret = scm_cons(scm_cons(scm_from_utf8_string(name), scm_from_utf8_string(value)),
		       ret);
    }
    return scm_reverse(ret);
}
...
{
    GIBaseInfo *c_info = _scm_to_gibaseinfo(s_info);
    return scm_from_int(g_callable_info_get_instance_ownership_transfer(c_info));
}
...
gig_callable_info_get_return_attributes (SCM s_info)
{
    ...
    char *value;
    SCM ret = SCM_EOL;
    while (g_callable_info_iterate_return_attributes (c_info, &iter, &name, &value))
    {
	ret = scm_cons(scm_cons(scm_from_utf8_string(name), scm_from_utf8_string(value)),
		       ret);
    }
...
SCM
gig_arg_info_get_ownership_scope(SCM s_info)
{
    GIBaseInfo *c_info = _scm_to_gibaseinfo(s_info);
...
gig_constant_info_get_value(SCM s_info)
{
    ...
    siz = g_constant_info_get_value(c_info, &value);
    switch (typetag) {
    ...
    case GI_TYPE_TAG_UTF8:
        ...
        break;
    default:
        scm_misc_error("constant-info-get-value", "unsupported constant type '~a'",
		       scm_list_1(scm_from_utf8_string(g_type_tag_to_string(typetag))));
...
gig_init_girepository ()
{
	...
	gig_TYPE_TAG_INTERFACE = flag("TYPE_TAG_INTERFACE", GI_TYPE_TAG_INTERFACE);
	...
	gig_ARRAY_TYPE_PTR_ARRAY = flag("ARRAY_TYPE_PTR_ARRAY", GI_ARRAY_TYPE_PTR_ARRAY);
	gig_ARRAY_TYPE_BYTE_ARRAY = flag("ARRAY_TYPE_BYTE_ARRAY", GI_ARRAY_TYPE_BYTE_ARRAY);
	...
	gig_INFO_TYPE_FUNCTION = flag("INFO_TYPE_FUNCTION", GI_INFO_TYPE_FUNCTION);
	gig_INFO_TYPE_CALLBACK = flag("INFO_TYPE_CALLBACK", GI_INFO_TYPE_CALLBACK);
	...
	gig_INFO_TYPE_INTERFACE = flag("INFO_TYPE_INTERFACE", GI_INFO_TYPE_INTERFACE);
	gig_INFO_TYPE_CONSTANT = flag("INFO_TYPE_CONSTANT", GI_INFO_TYPE_CONSTANT);
	gig_INFO_TYPE_INVALID_0 = flag("INFO_TYPE_INVALID_0", GI_INFO_TYPE_INVALID_0);
	...
	gig_INFO_TYPE_PROPERTY = flag("INFO_TYPE_PROPERTY", GI_INFO_TYPE_PROPERTY);
	...
	gig_INFO_TYPE_UNRESOLVED = flag("INFO_TYPE_UNRESOLVED", GI_INFO_TYPE_UNRESOLVED);
	...
	scm_c_define_gsubr ("callable-info-get-instance-ownership-transfer", 1, 0, 0,
	...
	gig_FUNCTION_IS_METHOD = flag("FUNCTION_IS_METHOD", GI_FUNCTION_IS_METHOD);
	gig_FUNCTION_IS_CONSTRUCTOR = flag("FUNCTION_IS_CONSTRUCTOR", GI_FUNCTION_IS_CONSTRUCTOR);
	gig_FUNCTION_IS_GETTER = flag("FUNCTION_IS_GETTER", GI_FUNCTION_IS_GETTER);
	...
	gig_VFUNC_MUST_CHAIN_UP = flag("VFUNC_MUST_CHAIN_UP", GI_VFUNC_MUST_CHAIN_UP);
	gig_VFUNC_MUST_OVERRIDE = flag("VFUNC_MUST_OVERRIDE", GI_VFUNC_MUST_OVERRIDE);
	gig_VFUNC_MUST_NOT_OVERRIDE = flag("VFUNC_MUST_NOT_OVERRIDE", GI_VFUNC_MUST_NOT_OVERRIDE);
