...
gig_argument_scm_to_c(S2C_ARG_DECL)
{
    ...
    *must_free = GIR_FREE_NONE;
    ...
    if (entry->may_be_null && scm_is_false(obj)) {
        arg->v_pointer = NULL;
        return;
    }
...
scm_to_c_immediate(S2C_ARG_DECL)
{
    switch (entry->type_tag) {
    case GI_TYPE_TAG_INT8:
        if (!scm_is_signed_integer(obj, INT8_MIN, INT8_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "int8");
        arg->v_int8 = scm_to_int8(obj);
        break;
    case GI_TYPE_TAG_UINT8:
        if (!scm_is_unsigned_integer(obj, 0, UINT8_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "uint8");
        arg->v_uint8 = scm_to_uint8(obj);
        break;
    case GI_TYPE_TAG_INT16:
        if (!scm_is_signed_integer(obj, INT16_MIN, INT16_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "int16");
        arg->v_int16 = scm_to_int16(obj);
        break;
    case GI_TYPE_TAG_UINT16:
        if (!scm_is_unsigned_integer(obj, 0, UINT16_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "uint16");
        arg->v_uint16 = scm_to_uint16(obj);
        break;
    case GI_TYPE_TAG_INT32:
        if (!scm_is_signed_integer(obj, INT32_MIN, INT32_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "int32");
        arg->v_int32 = scm_to_int32(obj);
        break;
    case GI_TYPE_TAG_UINT32:
        if (!scm_is_unsigned_integer(obj, 0, UINT32_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "uint32");
        arg->v_uint32 = scm_to_uint32(obj);
        break;
    case GI_TYPE_TAG_INT64:
        if (!scm_is_signed_integer(obj, INT64_MIN, INT64_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "int64");
        arg->v_int64 = scm_to_int64(obj);
        break;
    case GI_TYPE_TAG_UINT64:
        if (!scm_is_unsigned_integer(obj, 0, UINT64_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "uint64");
        arg->v_uint64 = scm_to_uint64(obj);
        break;
    case GI_TYPE_TAG_BOOLEAN:
        arg->v_boolean = scm_is_true(obj);
        break;
    case GI_TYPE_TAG_FLOAT:
        if (!scm_is_real(obj))
            scm_wrong_type_arg_msg(subr, argnum, obj, "float32");
        double dtmp = scm_to_double(obj);
    ...
    case GI_TYPE_TAG_DOUBLE:
        if (!scm_is_real(obj))
            scm_wrong_type_arg_msg(subr, argnum, obj, "float64");
        arg->v_double = scm_to_double(obj);
        break;
    case GI_TYPE_TAG_GTYPE:
        if (!scm_is_unsigned_integer(obj, 0, UINT64_MAX))
            scm_wrong_type_arg_msg(subr, argnum, obj, "GType integer");
        arg->v_size = scm_to_size_t(obj);
        break;
    case GI_TYPE_TAG_UNICHAR:
        if (SCM_CHARP(obj))
            arg->v_uint32 = SCM_CHAR(obj);
        else if (scm_is_unsigned_integer(obj, 0, SCM_CODEPOINT_MAX))
            arg->v_uint32 = scm_to_uint32(obj);
        else {
            scm_wrong_type_arg_msg(subr, argnum, obj, "char");
        }
...
scm_to_c_interface(S2C_ARG_DECL)
{
    ...
    if (referenced_base_type == GI_INFO_TYPE_ENUM)
        arg->v_int = scm_to_int(obj);
    else if (referenced_base_type == GI_INFO_TYPE_FLAGS)
        arg->v_uint = scm_to_uint(obj);
        ...
        if (scm_is_true(scm_procedure_p(obj))) {
            ...
            if (arity == n_args) {
                ...
                scm_wrong_type_arg_msg(subr, argnum, obj, str);
            }
        }
    }
...
scm_to_c_immediate_pointer(S2C_ARG_DECL)
{
    ...
    if (!scm_is_bytevector(obj)) {
        scm_wrong_type_arg_msg(subr, argnum, obj, "a bytevector");
    }
    else {
...
scm_to_c_string(S2C_ARG_DECL)
{
    ...
    if (scm_is_bytevector(obj)) {
        ...
        if (entry->transfer == GI_TRANSFER_NOTHING) {
            ...
            for (size_t i = 0; i < SCM_BYTEVECTOR_LENGTH(obj); i++)
                if (SCM_BYTEVECTOR_CONTENTS(obj)[i] == 0)
                    terminated = TRUE;
            if (!terminated)
                scm_wrong_type_arg_msg(subr, argnum, obj, "null-terminated bytevector");
            arg->v_string = (char *)SCM_BYTEVECTOR_CONTENTS(obj);
        }
        else
        ...
        scm_wrong_type_arg_msg(subr, argnum, obj, "string or bytevector");
}
...
scm_to_c_void_pointer(S2C_ARG_DECL)
{
    if (SCM_POINTER_P(obj))
        ...
        scm_wrong_type_arg_msg(subr, argnum, obj, "pointer");
}
...
scm_to_c_interface_pointer(S2C_ARG_DECL)
{
    ...
    if (obj_type == G_TYPE_NONE || obj_type == G_TYPE_INVALID)
        scm_wrong_type_arg_msg(subr, argnum, obj, "a GObject struct, union, interface, or object");
    ...
    if (!g_type_is_a(obj_type, arg_type)) {
        ...
        scm_wrong_type_arg_msg(subr, argnum, obj, msg);
    }
    else if ((referenced_base_type == GI_INFO_TYPE_STRUCT)
             || (referenced_base_type == GI_INFO_TYPE_UNION)
             || (referenced_base_type == GI_INFO_TYPE_OBJECT)) {
...
#define FUNC_NAME "%object->c-native-immediate-array-arg"
    ...
    if (scm_is_bytevector(obj)) {
        *size = SCM_BYTEVECTOR_LENGTH(obj);
        if (entry->item_transfer == GI_TRANSFER_NOTHING) {
            if (!entry->array_is_zero_terminated) {
                ...
                size_t len = SCM_BYTEVECTOR_LENGTH(obj);
                // Adding null terminator element.
                arg->v_pointer = g_malloc0(len + entry->item_size);
                memcpy(arg->v_pointer, SCM_BYTEVECTOR_CONTENTS(obj), len);
                *must_free = GIR_FREE_SIMPLE;
            }
        }
        ...
        scm_wrong_type_arg_msg(subr, argnum, obj, "bytevector");
#undef FUNC_NAME
}
...
{
    if (scm_is_bytevector(obj)) {
        void *contents = SCM_BYTEVECTOR_CONTENTS(obj);
        size_t len = SCM_BYTEVECTOR_LENGTH(obj);
        *size = len;
        if (entry->transfer == GI_TRANSFER_EVERYTHING)
...
#define FUNC_NAME "%object->c-garray-array-arg"
    scm_misc_error(FUNC_NAME,
                   "Marshalling to C GArray pointer args is unimplemented: ~S", scm_list_1(obj));
#undef FUNC_NAME
}
...
scm_to_c_native_direct_struct_array(S2C_ARG_DECL)
{
    ...
    size_t len = scm_to_size_t(scm_length(obj));
    gpointer ptr;
    if (entry->array_is_zero_terminated)
        ptr = g_malloc0_n(entry->item_size, len + 1);
    else
        ptr = g_malloc0_n(entry->item_size, len);
    gpointer entry_ptr = gi_gobject_get_obj(obj);
...
#define FUNC_NAME "%object->c-native-interface-array-arg"
    if ((entry->referenced_base_type == GI_INFO_TYPE_ENUM)
        ...
        if (!entry->item_is_ptr && entry->referenced_base_type == GI_INFO_TYPE_STRUCT)
            object_to_c_native_direct_struct_array_arg(subr, argnum, obj, ai, arg);
        else if (entry->item_is_ptr && entry->referenced_base_type == GI_INFO_TYPE_STRUCT)
            object_to_c_native_indirect_struct_array_arg(subr, argnum, obj, ai, arg);
        else if (!entry->item_is_ptr && entry->referenced_base_type == GI_INFO_TYPE_UNION)
            object_to_c_native_direct_union_array_arg(subr, argnum, obj, ai, arg);
        else if (entry->item_is_ptr && entry->referenced_base_type == GI_INFO_TYPE_UNION)
            object_to_c_native_indirect_union_array_arg(subr, argnum, obj, ai, arg);
            ...
            object_to_c_native_direct_object_array_arg(subr, argnum, obj, ai, arg);
        else if (entry->item_is_ptr && entry->referenced_base_type == G_TYPE_OBJECT)
            object_to_c_native_indirect_object_array_arg(subr, argnum, obj, ai, arg);
#endif
    }
    else {
...
scm_to_c_native_string_array(S2C_ARG_DECL)
{
    ...
    if (scm_is_true(scm_list_p(obj))) {
        ...
        *size = len;
        ...
        elt = scm_vector_elements(obj, &handle, &len, &inc);
        *size = len;
        gchar **strv = g_new0(gchar *, len + 1);
        ...
        scm_wrong_type_arg_msg(subr, argnum, obj, "list or vector of strings");
}
...
c_interface_pointer_to_scm(C2S_ARG_DECL)
{
    ...
    if (referenced_info_type == GI_INFO_TYPE_ENUM) {
        ...
        *obj = scm_from_pointer(callback_ptr, NULL);
        ...
        g_assert_nonnull(arg->v_pointer);
        GType gtype = g_registered_type_info_get_g_type(referenced_base_info);
        *obj = gir_type_make_object(gtype, arg->v_pointer, entry->transfer);
    }
    g_base_info_unref(referenced_base_info);
}
...
c_native_array_to_scm(C2S_ARG_DECL)
{
    ...
    if (SCM_UNBNDP(*obj) && entry->item_size) {
        size_t sz = length * entry->item_size;
        // FIXME: maybe return a typed vector or list
        *obj = scm_c_make_bytevector(sz);
        memcpy(SCM_BYTEVECTOR_CONTENTS(*obj), arg->v_pointer, sz);
    }
}
    ...
    *obj = scm_make_list(scm_from_size_t(length), SCM_UNDEFINED);
    ...
    while (list != NULL) {
        ...
        if (!item_is_ptr) {
            ...
            gig_argument_c_to_scm(subr, argnum, ae, &_arg, must_free, &elt, -1);
