                     ...
                     void *hint, void *marshal_data)
{
    ...
    for (unsigned i = 0; i < n_params; i++, iter = scm_cdr(iter))
        scm_set_car_x(iter, value_as_scm(params + i, TRUE));
    SCM _ret = scm_apply_0(pc->callback, args);

    if (G_IS_VALUE(ret) && value_from_scm(ret, scm_c_value_ref(_ret, 0)) != 0) {
...
closure_new(SCM callback, SCM inout_mask)
{
    ...
    if (SCM_UNBNDP(inout_mask) || scm_is_false(inout_mask))
        ((Closure *)closure)->inout_mask = SCM_UNDEFINED;
    else
        ((Closure *)closure)->inout_mask = scm_gc_protect_object(inout_mask);
    return closure;
}
...
invoke_closure(SCM closure, SCM return_type, SCM inout_mask, SCM args)
{
    ...
    for (size_t narg = 0; narg < nargs; narg++, iter = scm_cdr(iter)) {
        const GValue *arg = peek_typed_object(scm_car(iter), get_value_type());
        if (arg == NULL) {
            free(retval);
            goto out;
        }
    ...
    ret = transfer_object(G_TYPE_VALUE, retval, GI_TRANSFER_EVERYTHING);
...
procedure_to_closure(SCM procedure, SCM inout_mask)
{
    ...
    GClosure *cls = closure_new(procedure, inout_mask);
    g_closure_ref(cls);
    g_closure_sink(cls);
    return transfer_object(G_TYPE_CLOSURE, cls, GI_TRANSFER_EVERYTHING);
}
