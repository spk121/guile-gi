...
#include "closure.h"
#include "value.h"
#include "y/function_private.h"
#include "y/guile.h"

GQuark user_object_properties;

typedef struct _Gig_user_object_init_info
{
    GType type;
    ...
    Signal_spec **signals;
    size_t n_signals;
} Gig_user_object_init_info;

static SCM
take_object(GObject *object)
{
    return transfer_object(G_OBJECT_TYPE(object), object, GI_TRANSFER_EVERYTHING);
}
...
{
    return transfer_object(G_OBJECT_TYPE(object), object, GI_TRANSFER_NOTHING);
}

static GObject *
object_peek(SCM object)
{
    return peek_typed_object(object, object_type);
}

static GParamSpec *
peek_paramspec(SCM object)
{
    return peek_typed_object(object, paramspec_type);
}

static SCM
i_scm_make_gobject(SCM s_gtype, SCM s_prop_keylist)
{
#define FUNC "%make-gobject"
    ...
    if (SCM_UNBNDP(get_scheme_type(type)))
    ...
    if (scm_is_list(s_prop_keylist) && (scm_c_length(s_prop_keylist) != 0)) {
        ...
        keys = dynfree(calloc(n_prop, sizeof(char *)));
        values = dynfree(calloc(n_prop, sizeof(GValue)));
        ...
        for (size_t i = 0; i < n_prop; i++, iter = scm_cddr(iter)) {
            ...
            keys[i] = dynfree(scm_to_utf8_string(key));
            ...
            if (!pspec) {
                ...
                if (value_from_scm(value, s_value))
...
static SCM
i_scm_get_pspec(SCM self, SCM prop)
{
    GObject *obj;
    char *name;
    GParamSpec *pspec;

    SCM_ASSERT(SCM_IS_A_P(self, object_type), self, SCM_ARG1, "%get-pspec");

    scm_dynwind_begin(0);
    obj = object_peek(self);
    name = dynfree(scm_to_utf8_string(prop));
    ...
    return transfer_object(G_PARAM_SPEC_TYPE(pspec), pspec, GI_TRANSFER_NOTHING);
}

static SCM
i_scm_get_property(SCM self, SCM property)
{
    SCM ret;
    ...
    SCM_ASSERT(SCM_IS_A_P(self, object_type), self, SCM_ARG1, "%get-property");
    SCM_ASSERT(SCM_IS_A_P(property, paramspec_type), property, SCM_ARG2, "%get-property");
    obj = peek_object(self);
    pspec = peek_paramspec(property);
    ...
    ret = value_param_as_scm(&value, TRUE, pspec);
    g_value_unset(&value);
...
static SCM
i_scm_set_property_x(SCM self, SCM property, SCM svalue)
{
    GParamSpec *pspec;
    GObject *obj;
    ...
    SCM_ASSERT(SCM_IS_A_P(self, object_type), self, SCM_ARG1, "%set-property!");
    SCM_ASSERT(SCM_IS_A_P(property, paramspec_type), property, SCM_ARG2, "%set-property");
    ...
    g_value_init(&value, G_PARAM_SPEC_VALUE_TYPE(pspec));
    value_from_scm_with_error(&value, svalue, "%set-property!", SCM_ARG3);
...
static void
make_new_signal(Signal_spec *signal_spec, GType instance_type)
{
...
static void
get_user_object_property(GObject *object, unsigned property_id, GValue *value,
                             GParamSpec *pspec)
{
    GValue *properties = g_object_get_qdata(object, user_object_properties);
    GValue *property = properties + property_id - 1;
    g_value_copy(property, value);
}

static void
set_user_object_property(GObject *object, unsigned property_id,
...
static void
dispose_user_object(GObject *object)
{
...
static void
finalize_user_object(GObject *object)
{
}

static void
init_user_class(GObjectClass *_class, void *class_info)
{
    GType type = G_TYPE_FROM_CLASS(_class);
    Gig_user_object_init_info *init_info = class_info;
    size_t n_properties = init_info->n_properties;
    GParamSpec **properties = init_info->properties;

    _class->set_property = set_user_object_property;
    _class->get_property = get_user_object_property;
    _class->dispose = dispose_user_object;
    _class->finalize = finalize_user_object;
...
static void
init_user_object(GTypeInstance *instance, void *class_ptr)
{
    ...
    g_object_set_qdata(G_OBJECT(instance), user_object_properties, instance_properties);
...
static GType
define_user_object(const char *type_name,
                       GType parent_type, GParamSpec **properties,
                       size_t n_properties, Signal_spec **signals, size_t n_signals)
{
    GTypeInfo type_info;
    Gig_user_object_init_info *class_init_info;
    GTypeQuery query;
    GType new_type;
    ...
    class_init_info = xcalloc(1, sizeof(Gig_user_object_init_info));
    ...
    type_info.class_init = (GClassInitFunc) init_user_class;
    type_info.instance_init = init_user_object;
...
static SCM
i_scm_define_type(SCM s_type_name, SCM s_parent_type, SCM s_properties, SCM s_signals)
{
    char *type_name;
    GType parent_type;
    GType new_type;
    ...
    SCM_ASSERT(SCM_SUBCLASSP(s_parent_type, object_type), s_parent_type, SCM_ARG2,
               "%define-type");
    ...
    if (SCM_UNBNDP(get_scheme_type(parent_type)))
    ...
    if (scm_is_list(s_properties)) {
        ...
        for (size_t i = 0; i < n_properties; i++) {
            GParamSpec *pspec = peek_paramspec(scm_car(iter));
            properties[i] = pspec;
            iter = scm_cdr(iter);
        }
    }
    ...
    if (scm_is_list(s_signals)) {
        ...
        signals = xcalloc(n_signals, sizeof(Signal_spec *));
        for (size_t i = 0; i < n_signals; i++) {
            Signal_spec *sspec = NULL;
            sspec = signalspec_from_obj(scm_c_list_ref(s_signals, i));
            signals[i] = sspec;
        }
    }
    else {
    ...
    new_type =
        define_user_object(type_name, parent_type, properties, n_properties, signals,
                               n_signals);
    free(type_name);
    define_type(new_type, SCM_UNDEFINED);
    return get_scheme_type(new_type);
}
    ...
    *c_signal = g_signal_lookup(name, G_OBJECT_TYPE(self));
    ...
    if (c_signal == 0)
        scm_misc_error(proc, "~A: unknown signal name ~A",
                       scm_list_2(object_ref(self), s_name));
...
static SCM
i_scm_connect(SCM self, SCM signal, SCM sdetail, SCM callback, SCM s_after, SCM reserved)
{
    GObject *obj;
    int after;
    GClosure *closure;
    ...
    SCM_ASSERT(SCM_IS_A_P(self, object_type), self, SCM_ARG1, "%connect");
    SCM_ASSERT(SCM_IS_A_P(signal, signal_type), signal, SCM_ARG2, "%connect");
    ...
    closure = closure_new(callback, signal_ref(signal, SIGNAL_SLOT_OUTPUT_MASK));
...
static SCM
i_scm_emit(SCM self, SCM signal, SCM s_detail, SCM args)
{
    GObject *obj;
    ...
    SCM_ASSERT(SCM_IS_A_P(self, object_type), self, SCM_ARG1, "%emit");
    SCM_ASSERT(SCM_IS_A_P(signal, signal_type), signal, SCM_ARG2, "%emit");
    ...
    value_from_scm_with_error(values, self, "%emit", SCM_ARG1);
    SCM iter = args;
    for (unsigned i = 0; i < query_info.n_params; i++, iter = scm_cdr(iter)) {
        g_value_init(values + i + 1, query_info.param_types[i]);
        value_from_scm_with_error(values + i + 1, scm_car(iter), "%emit", SCM_ARGn);
    }
    ...
    if (query_info.return_type != G_TYPE_NONE)
        ret = scm_cons(value_as_scm(&retval, FALSE), ret);

    SCM output_mask = signal_ref(signal, SIGNAL_SLOT_OUTPUT_MASK);
    if (scm_is_bitvector(output_mask)) {
        ...
        for (unsigned i = 0; i < query_info.n_params + 1; i++, pos += inc) {
            ...
            if (bits[word_pos] & mask)
                ret = scm_cons(value_as_scm(values + i, FALSE), ret);
        }
...
SCM
define_property(GType type, GIPropertyInfo *info, const char *_namespace, SCM defs)
{
    ...
    SCM self_type = get_scheme_type(type);
    ...
    long_name = gname_to_scm_name(mid_name);
    free(mid_name);
    ...
    if (G_TYPE_IS_CLASSED(type)) {
        ...
        critical_load("%s is neither class nor interface, but we define properties, wtf?",
                          g_type_name(type));
    if (prop != NULL) {
        s_prop = transfer_object(G_PARAM_SPEC_TYPE(prop), prop, GI_TRANSFER_NOTHING);
...
init_object(void)
{
    user_object_properties = g_quark_from_static_string("GigObject::properties");

    sym_value = scm_from_utf8_symbol("value");

    ensure_accessor_proc = scm_c_public_ref("oop goops", "ensure-accessor");

    scm_c_define_gsubr("%make-gobject", 1, 1, 0, i_scm_make_gobject);
    scm_c_define_gsubr("%object-get-pspec", 2, 0, 0, i_scm_get_pspec);
    scm_c_define_gsubr("%get-property", 2, 0, 0, i_scm_get_property);
    scm_c_define_gsubr("%set-property!", 3, 0, 0, i_scm_set_property_x);
    scm_c_define_gsubr("%connect", 4, 2, 0, i_scm_connect);
    scm_c_define_gsubr("%emit", 2, 1, 1, i_scm_emit);
    scm_c_define_gsubr("%define-object-type", 2, 2, 0, i_scm_define_type);
}
