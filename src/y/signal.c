...
SCM
signal_ref(SCM signal, Signal_slot slot)
{
                ...
                GValue *seed, const GValue *element, void *procedure)
{
    ...
    _seed = value_as_scm(seed, FALSE);
    _element = value_as_scm(element, FALSE);
    ...
    case 1:
        if (!scm_is_eq(result, SCM_UNSPECIFIED))
            return_val_if_fail(!value_from_scm(seed, result), FALSE);
        return TRUE;
    case 2:
    {
        int ret = scm_is_true(scm_c_value_ref(result, 0));
        SCM next_seed = scm_c_value_ref(result, 1);
        return_val_if_fail(!value_from_scm(seed, next_seed), FALSE);
        return ret;
    }
    default:
    {
...
Signal_spec *
signalspec_from_obj(SCM obj)
{
    unsigned n_params;
    GType *params;
    SCM sparams, saccu;
    GSignalFlags flags = 0;
    Signal_spec *spec = NULL;

    SCM_ASSERT_TYPE(SCM_IS_A_P(obj, signal_type), obj, SCM_ARG1, "%scm->signalspec", "signal");

    scm_dynwind_begin(0);
    spec = xcalloc(1, sizeof(Signal_spec));
    scm_dynwind_unwind_handler((handler_func) free_signalspec, spec, 0);
    spec->return_type = scm_to_gtype(signal_ref(obj, SIGNAL_SLOT_RETURN_TYPE));
    if (spec->return_type == G_TYPE_INVALID)
        scm_misc_error("%scm->signalspec", "signal ~A has no return type",
                       scm_list_1(signal_ref(obj, SIGNAL_SLOT_NAME)));
    spec->signal_name = scm_to_utf8_string(signal_ref(obj, SIGNAL_SLOT_NAME));
    ...
    do {
        SCM _flags = signal_ref(obj, SIGNAL_SLOT_FLAGS);
        // accept #f as 0, otherwise use flags->number
        if (scm_is_true(_flags))
            flags = flags_to_uint(_flags);
    } while (0);
...
void
free_signalspec(Signal_spec *spec)
{
    if (spec) {
        if (spec->param_types) {
...
SCM
make_signal(size_t n_slots, Signal_slot *slots, SCM *slot_values)
{
...
init_signal()
{
    signal_type = scm_c_public_ref("gi oop", "<signal>");
    ...
    signal_slot_syms[SIGNAL_SLOT_OUTPUT_MASK] = scm_from_utf8_symbol("output-mask");
