...
{
    return glib_loaded && gobject_loaded;
}
                            ...
                            int *n_signals, Repository_nested *signal)
{
    ...
    case GI_INFO_TYPE_FLAGS:
        ...
        *method = (Repository_nested)g_enum_info_get_method;
        *n_properties = *n_signals = 0;
        *property = *signal = NULL;
        break;
    case GI_INFO_TYPE_INTERFACE:
        *n_methods = g_interface_info_get_n_methods(base);
        *method = (Repository_nested)g_interface_info_get_method;
        *n_signals = g_interface_info_get_n_signals(base);
        *signal = (Repository_nested)g_interface_info_get_signal;
        *n_properties = g_interface_info_get_n_properties(base);
        *property = (Repository_nested)g_interface_info_get_property;
        {
    ...
    case GI_INFO_TYPE_OBJECT:
        ...
        *method = (Repository_nested)g_object_info_get_method;
        *n_properties = g_object_info_get_n_properties(base);
        *property = (Repository_nested)g_object_info_get_property;
        *n_signals = g_object_info_get_n_signals(base);
        *signal = (Repository_nested)g_object_info_get_signal;
        break;
    default:
...
load_info(GIBaseInfo *info, LoadFlags flags, SCM defs)
{
    ...
    case GI_INFO_TYPE_SIGNAL:
        defs = define_function(parent_gtype, info, parent_name, defs);
        break;
    case GI_INFO_TYPE_PROPERTY:
        defs = define_property(parent_gtype, info, parent_name, defs);
        break;
    ...
    case GI_INFO_TYPE_OBJECT:
    {
        ...
        int has_parent = p ? TRUE : FALSE;
        if (p)
            g_base_info_unref(p);
        if (!has_parent) {
            debug_load("%s:%s - has no parent", _namespace, g_type_name(gtype));
            define_fundamental_type(gtype, SCM_EOL, g_object_ref_sink, g_object_unref);
        }
        defs = define_type(gtype, defs);
        goto recursion;
    }
    ...
    case GI_INFO_TYPE_CONSTANT:
        defs = define_constant(info, defs);
        break;
    case GI_INFO_TYPE_UNION:
    {
        GType gtype = g_registered_type_info_get_g_type(info);
...
#define LOAD_NESTED(F, N, I)                                    \
        ...
        Repository_nested method, property, nested_signal;
...
scm_find_entry_by_name(SCM lib, SCM name)
{
    ...
    scm_dynwind_begin(0);
    _lib = dynfree(scm_to_utf8_string(lib));
    _name = dynfree(scm_to_utf8_string(name));
