...
#define TRACE_C2S() debug_transfer("[C2S] On line %d while handing %s of %s.", __LINE__, meta_type_describe(meta), subr)
#define TRACE_S2C() debug_transfer("[S2C] On line %d while handing %s of %s.", __LINE__, meta_type_describe(meta), subr)

#define SURPRISING                                                      \
    do {                                                                \
        warning_transfer("Unusual argument type '%s' %s:%d", meta_type_describe(meta), __FILE__, __LINE__); \
    } while(FALSE)

#define UNHANDLED                                                       \
    do {                                                                \
        error_transfer("unhandled argument type '%s' %s:%d", meta_type_describe(meta), __FILE__, __LINE__); \
    } while(FALSE)
...
#if MOVE_TO_Z
static gtype_t
child_type(MetaType *meta, GIArgument *arg)
{
...
static size_t
zero_terminated_array_length(MetaType *meta, GIArgument *arg)
{
    assert(meta != NULL);
    assert(arg != NULL);
    ...
    if (arg->v_pointer == NULL)
        return 0;
    else if (meta->params[0].payload == META_UTF8_STRING
             || meta->params[0].payload == META_LOCALE_STRING)
...
argument_scm_to_c(S2C_ARG_DECL)
{
    ...
    case META_DOUBLE:
        ...
        break;
    case META_UTF8_STRING:
    case META_LOCALE_STRING:
...
{
    TRACE_S2C();
    gtype_t fundamental_type = G_TYPE_FUNDAMENTAL(meta->gtype);
...
{
    TRACE_S2C();
    gtype_t item_type = meta->params[0].gtype;
    gtype_t fundamental_item_type = G_TYPE_FUNDAMENTAL(item_type);
