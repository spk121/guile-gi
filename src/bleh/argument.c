...
{
    TRACE_S2C();
    // For booleans, we expect a vector of booleans.  This must be the
    // GLib int size booleans, not stdbool.
...
// This procedure converts a GIArgument to a pointer to be used as a
...
#undef DO_TRANSFER
    ...
    case G_TYPE_BOOLEAN:
    {
        *object = scm_c_make_vector(length, SCM_BOOL_F);
        ...
        // Must be int-sized gboolean, not stdbool.
