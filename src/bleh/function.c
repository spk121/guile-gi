...
make_cache(void)
{
    Cache *kv = xcalloc(1, sizeof(Cache));
    kv->entries = xcalloc(20, sizeof(CacheItem));
    kv->alloc = 20;
    kv->len = 0;
    return kv;
}
...
static int
item_comparison(const void *pA, const void *pB)
{
    const CacheItem *A = pA;
    const CacheItem *B = pB;
    return (A->item > B->item) - (A->item < B->item);
}

static Function *
find_func(Cache *cache, GIFunctionInfo *info)
{
    void *pB;
    CacheItem A;
    A.key = key;
    ...
    pB = bsearch(&A, kv->entries, kv->len, sizeof(CacheItem), item_comparison);
    if (pB == NULL)
        return 0;

    return ((CacheItem *) pB)->func;
}

static void
cache_func(Cache *cache, GIFunctionInfo *info, Function *func)
{
    CacheItem A;
    A.info = info;
    A.func = func;
    ...
    if (pB == NULL) {
        if (kv->alloc == kv->len) {
            kv->alloc += 20;
            kv->entries = realloc(kv->entries, kv->alloc * sizeof(CacheItem));
        }
        kv->entries[kv->len].info = info;
        kv->entries[kv->len].func = func;
        kv->len++;
        qsort(kv->entries, kv->len, sizeof(CacheItem), item_comparison);
        return;
    }
    
    debug_load("multiple functions defined for a single function into");
}
...
free_cache(Cache *cache)
{
    for (int i = 0; i < cache->len; i++) {
        function_free(sv->entries[i].func);
    }
    free(cache->entries);
    free(cache);
}
...
static void function_free(Function *fn);
...
gig_function_define(GType type, GICallableInfo *info, const char *_namespace, SCM defs)
{
    ...
    function_name = dynwind_free(make_name(info, _namespace));
    ...
    if (is_method) {
        ...
        method_name = dynwind_free(gig_callable_info_make_name(info, NULL));
            ...
            SCM *specializers)
{
    ...
    for (int i = 1; i < *req + *opt; i++, pos += inc) {
        ...
        GigArgMapEntry *entry = arg_map_get_input_entry_by_s(amap, i - 1);
                  ...
                  int *optional_input_count, SCM *formals, SCM *specializers)
{
    ...
    Function *gfn =
        find_func(function_cache, function_info);
             ...
             GigArgMap *argmap, int n_inputs, SCM self_type, SCM *formals, SCM *specializers)
{
    ...
    for (int s = 0; s < n_inputs;
        ...
        GigArgMapEntry *entry = arg_map_get_input_entry_by_s(argmap, s);
             ...
             int *required_input_count, int *optional_input_count, SCM *formals, SCM *specializers)
{
    ...
    cache_func(function_cache, function_info, gfn);
...
{
    debug_load("Freeing functions");
    free_cache(function_cache, NULL, function_free);
    function_cache = NULL;
}
