...
#define gig_debug_amap(x,...) printf(x,__VA_ARGS__)
...
static ArgMap *arg_map_allocate(size_t n);
static void arg_map_apply_function_info(ArgMap *amap, GIFunctionInfo *func_info);
static void arg_map_determine_argument_presence(ArgMap *amap, GIFunctionInfo *info);
static void arg_map_compute_c_invoke_positions(ArgMap *amap);
static void arg_map_compute_s_call_positions(ArgMap *amap);
static void arg_map_entry_init(ArgMapEntry *map);

static void
arg_map_entry_init(ArgMapEntry *entry)
{
    memset(entry, 0, sizeof(ArgMapEntry));
...
arg_map_allocate(size_t n)
{
    ...
    amap = calloc(1, sizeof(ArgMap));
    amap->pdata = calloc(n, sizeof(ArgMapEntry));
    amap->len = n;
...
static void
arg_map_apply_function_info(ArgMap *amap, GIFunctionInfo *func_info)
{
    int i, n;
    GIArgInfo *arg_info;
    ...
    for (i = 0; i < n; i++) {
        ...
        meta_type_init_from_arg_info(&amap->pdata[i].meta, arg_info);
    ...
    meta_type_init_from_callable_info(&amap->return_val.meta, func_info);
...
static void
arg_map_determine_array_length_index(ArgMap *amap, ArgMapEntry *entry, GITypeInfo *info)
{
    if (entry->meta.payload == META_ARRAY && entry->meta.has_size) {
...
static void
arg_map_determine_argument_presence(ArgMap *amap, GIFunctionInfo *info)
{
    ArgMapEntry *entry;
    bool opt_flag = true;
    int i, n;
...
static void
arg_map_compute_c_invoke_positions(ArgMap *amap)
{
    int i, n;
    ArgMapEntry *entry;
    n = amap->len;
...
static void
arg_map_compute_s_call_positions(ArgMap *amap)
{
    int i, n;
    ArgMapEntry *entry;
    n = amap->len;
...
void
arg_map_free(ArgMap *amap)
{
    if (!amap)
        return;

    for (int i = 0; i < amap->len; i++) {
        meta_type_free(&amap->pdata[i].meta);
        free(amap->pdata[i].name);
    }
    meta_type_free(&amap->return_val.meta);
    free(amap->return_val.name);
    free(amap->pdata);
    free(amap->name);
    amap->pdata = NULL;
...
void
arg_map_dump(const char *name, const ArgMap *amap)
{
    ...
    for (int i = 0; i < amap->len; i++) {
        const ArgMapEntry *entry = &amap->pdata[i];
        GString *s = g_string_new(NULL);
        g_string_append_printf(s, "  Arg %d: '%s' %s",
                               i, entry->name, meta_type_describe(&entry->meta));
        ...
        if (amap->pdata[i].meta.n_params > 0) {
            MetaType *m2 = &amap->pdata[i].meta.params[0];
            s = g_string_new(NULL);
            g_string_append_printf(s, "    Item Type: %s", meta_type_describe(m2));
            gig_debug_amap("%s", s->str);
            g_string_free(s, true);
        }
    }
    if (amap->return_val.meta.gtype != G_TYPE_NONE) {
        const ArgMapEntry *entry = &amap->return_val;
        GString *s = g_string_new(NULL);
        g_string_append_printf(s, "  Return: '%s' %s",
                               entry->name, meta_type_describe(&entry->meta));
        g_string_append_printf(s, ", %s, %s, %s",
                               dir_strings[entry->s_direction],
                               tuple_strings[entry->tuple], presence_strings[entry->presence]);
        gig_debug_amap("%s", s->str);
        ...
        if (amap->return_val.meta.n_params > 0) {
            MetaType *m2 = &amap->return_val.meta.params[0];
            s = g_string_new(NULL);
            g_string_append_printf(s, "    Item Type: %s", meta_type_describe(m2));
            gig_debug_amap("%s", s->str);
            g_string_free(s, true);
...
void
arg_map_s_input_count(const ArgMap *amap, int *required, int *optional)
{
    assert(amap != NULL);
...
ArgMapEntry *
arg_map_get_input_entry_by_s(ArgMap *amap, int spos)
{
    int i = 0;
...
ArgMapEntry *
arg_map_get_output_entry_by_c(ArgMap *amap, int cpos)
{
    assert(amap != NULL);
...
// argument.
bool
arg_map_output_child_c(ArgMap *amap, int c_output_pos, int *cinvoke_output_array_size_index)
{
    assert(amap != NULL);
...
// gsubr call.
void
arg_map_c_count(const ArgMap *amap, int *c_input_len, int *c_output_len)
{
    assert(amap != NULL);
...
bool
arg_map_input_s_2_input_c(const ArgMap *amap, int s_input_pos, int *c_input_pos)
{
    assert(amap != NULL);
...
bool
arg_map_input_s_2_output_c(const ArgMap *amap, int s_input_pos, int *c_output_pos)
{
    assert(amap != NULL);
...
bool
arg_map_input_s_2_child_input_c(const ArgMap *amap, int s_input_pos, int *c_input_pos)
{
    assert(amap != NULL);
    assert(c_input_pos != NULL);
    ...
    while (i < amap->len) {
        if (amap->pdata[i].is_s_input && (amap->pdata[i].s_input_pos == s_input_pos)) {
            ArgMapEntry *child = amap->pdata[i].child;
            if (child) {
                if (child->is_c_input) {
...
bool
arg_map_input_s_2_child_output_c(const ArgMap *amap, int s_input_pos, int *c_output_pos)
{
    assert(amap != NULL);
    assert(c_output_pos != NULL);
...
bool
arg_map_input_i2c(const ArgMap *amap, int i, int *cpos)
{
    if (i < 0 || i >= amap->len)
        return false;
    if (!amap->pdata[i].is_c_input)
...
bool
arg_map_input_i2s(const ArgMap *amap, int i, int *spos)
{
    if (i < 0 || i >= amap->len)
        return false;
...
bool
arg_map_input_c2i(const ArgMap *amap, int cpos, int *i)
{
...
bool
arg_map_input_s2i(const ArgMap *amap, int spos, int *i)
{
...
bool
arg_map_input_c2s(const ArgMap *amap, int cpos, int *spos)
{
    if (cpos < 0 || cpos >= amap->c_input_len) {
        return false;
    }
...
bool
arg_map_input_s2c(const ArgMap *am, int spos, int *cpos)
{
...
bool
arg_map_output_i2c(const ArgMap *amap, int i, int *cpos)
{
    if (i < 0 || i >= amap->len)
        return false;
    if (!amap->pdata[i].is_c_output)
...
bool
arg_map_output_i2s(const ArgMap *amap, int i, int *spos)
{
    if (i < 0 || i >= amap->len)
        return false;
...
bool
arg_map_output_c2i(const ArgMap *amap, int cpos, int *i)
{
...
bool
arg_map_output_s2i(const ArgMap *amap, int spos, int *i)
{
...
bool
arg_map_output_c2s(const ArgMap *amap, int cpos, int *spos)
{
    if (cpos < 0 || cpos >= amap->c_output_len)
        return false;
    int j = 0;
...
bool
arg_map_output_s2c(const ArgMap *am, int spos, int *cpos)
{
...
bool
arg_map_child_i(const ArgMap *am, int i, int *ichild)
{
...
bool
arg_map_return_child_i(const ArgMap *am, int *ichild)
{
