...
;(irepository-require "Gio" "2.0")
;(define infos (irepository-get-infos "Gio"))
...
;; constant-info-get-value converts constant-info to value
;; (struct-set struct-name field-name val)
(define func-info-strdup #f)


(define (define-constant cur)
  (format #t "(define ~A ~S) ; constant ~A~%"
	  (constant-info-get-name cur)
	  (constant-info-get-value cur)
	  (constant-info-get-type cur)))

(define (define-struct cur)
  (let ((name (struct-info-get-name cur))
	(alignment (struct-info-get-alignment cur))
	(size (struct-info-get-size cur))
	(gtype? (struct-info-is-gtype-struct? cur))
	(fields (struct-info-get-fields cur)))
    ;; define a
...
(let loop ((cur (car infos))
  ...
  (cond
   ...
   ((function-info? cur)
    (format #t "function ~a~%" (function-info-get-name cur))
    (if (string=? (function-info-get-name cur) "strdup")
	(set! func-info-strdup cur))
    (when (function-info-is-getter? cur)
      (format #t " ; getter"))
    (when (function-info-is-setter? cur)
      (format #t " ; setter"))
   ...
   ((object-info? cur)
    ...
    (let ((methods (object-info-get-methods cur)))
      (for-each (lambda (cur)
		  (format #t "  method ~a~%" (function-info-get-name cur))
		  (when (function-info-is-getter? cur)
		    (format #t " ; getter"))
		  (when (function-info-is-setter? cur)
		    (format #t " ; setter"))
   ...
   ((struct-info? cur)
    ...
    (format #t "  foreign? ~A~%" (struct-info-is-foreign? cur))
    (let ((fields (struct-info-get-fields cur)))
      (for-each (lambda (x)
		  (format #t "   ~a~%" x))
		fields))
...
;; (format #t "func-info-strdup ~S~%" func-info-strdup)
					;(sleep 20)

 ;;     (display (function-invoke func-info-strdup "hello"))
