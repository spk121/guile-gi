 ...
 *         conversion
 *
 * converts a Guile object to a GType.  If strict is set, raises an 
...
GType
pyg_type_from_object_strict(SCM_OBJECT obj, gboolean strict)
{
    SCM_OBJECT gtype;
    GType type;

    g_return_val_if_fail (obj != (SCM_OBJECT) 0, 0);
	...
	return G_TYPE_LONG;
    else if (obj == SCM_BOOL_T || obj == SCM_BOOL_F)
	...
	gchar *name = scm_to_utf8_string (obj);
    ...
    /* If SCM is following GObject conventions, it is a struct
       with an 'ob_type' slot that holds the foreign object type
       used to create it.  If that type is GuGTypeWrapper_Type,
       then it has a slot named 'type' which is the GType */
 ...
 */
GType
pyg_type_from_object(SCM_OBJECT obj)
{
